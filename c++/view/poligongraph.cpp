...
poligonGraph::poligonGraph(grafico* g,poligono* p): graficElement(g), vCoord0(p->getpoint()), pol(p){}


void poligonGraph::draw(unsigned k){
    int p = 0 ;
    ...
    for(unsigned int i = 0; i<poligonGraph::vCoord0.size() - 1; i++){
        for(unsigned int j = i + 1; j<vCoord0.size(); j++){
           if(poligonGraph::vCoord0[j]->distanceTwoPoints(*poligonGraph::vCoord0[i]) == poligonGraph::pol->lato() || vCoord0.size() == 3){
               ...
               ++p;
               qDebug("creato e disegnato lato poligono");
           }
        }
    }
...
void poligonGraph::drawing(unsigned k){ //per tenere protected draw
